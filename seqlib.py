...
class Distmatrix(object):
    """Class representing distance matrix for set of aligned sequences. Methods to set and get dists"""
    ...
    @classmethod
    def from_distmatfile(cls, filename):
        """Constructor 1: Reads file (which is in own distmat textformat) and constructs Distmatrix object"""

        self = cls()

        # Note: Should add more careful parsing and error checking at some point: check all pairs are accounted for!
        dmatfile = open(filename)
        dmatdata = dmatfile.readlines()
        dmatfile.close()

        # Construct list of names, and 2D list of values
        names = dmatdata[0].split()
        values = []
    ...
    def from_distdict(cls, distdict):
        """Constructor 3: constructs Distmatrix object from dictionary of {(name1, name2):dist}"""
        # Note: Should add more careful parsing and error checking at some point: check all pairs are accounted for!
        self = cls()
    ...
    def getdist(self, name1, name2):
        """Returns distance between sequences with names name1 and name2"""
    ...
    def getnames(self):
        """Returns list with all names in distance matrix"""
    ...
    def pairdists(self):
        """Returns all pairwise distances as string. One distance per line in format: name1 name2 distance"""
        ...
        for i in range(nseqs):
            ...
            for j in range(i+1, nseqs):
                ...
                distlist.append("%s\t%s\t%f" % (seq1, seq2, dist))
                distlist.append("\n")
        distlist.pop()
